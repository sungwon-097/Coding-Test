...
```cpp
string::npos와 비교하여 find가 찾았는지 못찾았는지 알 수 있었음
여러 문자가 바뀌는걸 대비해 while문으로 replace했음
regex_replace라는 C#같은게 있더라 - 얘는 존재하는지 검사할 필요 없어보임
코드가 엄청 줄어듬
```
...
```cpp
substr응용을 잘 해야되겠음
map으로는 [key]값으로 value에 접근할 수 있음
왠만하면 foreach처럼 쓰지말고 for i 쓰자 너무 헷갈린다
```
